...
export class VCustomerInvoice extends VPage<CInvoice> {
	...
	init() {
		let {customerPendingInvoice} = this.controller;
		this.invoiceForm = new InvoiceForm(this.controller, {detail: customerPendingInvoice}, this.submit);
	}
	header() {return '客户开票'}
	content() {
		return <div className="my-3">
			{this.invoiceForm.render()}
		</div>
	}

	private submit = async () => {
		let {customer, uqs} = this.controller;
		let details = this.invoiceForm.getDetails();
		await this.controller.doneInvoice(details);
		this.closePage();
		this.openPageElement(<Page header="开票完成" back="close">
			<div className="px-3 my-2">{uqs.JkCustomer.Customer.tv(customer)}</div>
			<div className="m-3">
				开票金额：<span className="fs-3 text-success">
					{currencyFormat.format(this.invoiceForm.sumAmount)}
				</span>
			</div>
		</Page>);
	}
...
class InvoiceForm extends InputForm<any> {
	private controller: CInvoice;
	private detailInputList: InputList<CustomerPendingInvoice, FormDetail>;
	private submit: () => Promise<void>;

	constructor(controller: CInvoice, values: {detail: CustomerPendingInvoice[]}, submit: () => Promise<void>) {
		super(values);
		makeObservable(this, {
	...
	get sumAmount(): number {
		return this.detailInputList?.itemForms.reduce((sum, itemForm) => {
			if (itemForm.selected === true) {
				return sum + shouldInvoice(itemForm.values);
			}
			return sum;
		}, 0);
	}
...
class FormDetail extends InputForm<CustomerPendingInvoice> {
	private controller: CInvoice;
	selected: boolean = false;
	constructor(controller: CInvoice, item: CustomerPendingInvoice) {
		super(item);
		makeObservable(this, {
			selected: observable,
		})
	...
	render() {
		...
		let right = <div>{currencyFormat.format(shouldInvoice(this.values))}</div>;
